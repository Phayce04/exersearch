...
function sameSetNums(a, b) {
  const A = [...uniqFiniteNums(a)].sort((x, y) => x - y);
  const B = [...uniqFiniteNums(b)].sort((x, y) => x - y);
...
function iconForAmenityName(name) {
  ...
  if (s.includes("aircon") || s.includes("air con") || s.includes("ac") || s.includes("vent") || s.includes("fan"))
    return Wind;
...
export default function UpdateAmenities({ gymId, existingAmenities = [], onClose, onSuccess }) {
  ...
  const hasChanges = useMemo(() => !sameSetNums(selectedIds, existingIds), [selectedIds, existingIds]);
  ...
  const addCustomAmenity = async () => {
    ...
    try {
      ...
      const nextAll = [...allAmenities, created].sort((a, b) => String(a?.name || "").localeCompare(String(b?.name || "")));
      setAllAmenities(nextAll);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(e?.message || "Failed to update amenities");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="modal-content amenities-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="modal-form">
          ...
          <div className="amenities-summary">
            ...
            <div className="summary-stat">
              <strong>{existingIds.length}</strong>
              <span>Currently Saved</span>
            </div>
          </div>

          <div className="form-actions">
            <button type="button" className="btn-secondary" onClick={handleReset} disabled={saving || !hasChanges}>
              Reset Changes
            </button>
            ...
            <div className="action-group">
              ...
              <button type="submit" className="btn-primary" disabled={saving || !hasChanges || selectedIds.length === 0}>
                {saving ? (
                  <>
